...
- Семантический реранжинг результатов
...
7. **Параллельное выполнение агентов** (`execute_agents_enhanced_node`)
   ...
   **Логика работы агентов:**
   ...
   - Максимум шагов на задачу: 5 (настраивается через `deep_research_agent_max_steps`)
   ...
   **Память агента:**
   - Агент видит последние 20 важных заметок из своего файла
...
8. **Координация через SupervisorQueue**
   ...
   **Максимум вызовов супервайзера:** 6 (настраивается через `deep_research_max_supervisor_calls`)
...
9. **Супервайзер координирует** (`supervisor_review_enhanced_node`)
   ...
   **Максимум итераций супервайзера:** 10 (настраивается через `deep_research_supervisor_max_iterations`)
...
11. **Генерация финального отчета** (`generate_final_report_enhanced_node`)
    ...
    **Процесс:**
    ...
    2. Если черновик слишком короткий или отсутствует - создание из всех находок
...
DEEP_RESEARCH_DEFAULT_MAX_ITERATIONS=15
...
| Память агентов | ❌ | ❌ | ❌ | ✅ |
| Уточняющие вопросы | ❌ | ❌ | ❌ | ✅ |
| Длина ответа | Средняя | 400-600 слов | 400-600 слов | 2000-5000+ слов |
...
Все режимы поддерживают Server-Sent Events (SSE) для:
- Реального времени обновлений прогресса
- Постепенной передачи ответа
